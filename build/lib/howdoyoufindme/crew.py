...
class HowDoYouFindMeCrew:
    ...
    def generate_keywords_task(self) -> Task:
        return Task(config=self.tasks_config["generate_keywords_task"])

    @task
    def build_query_task(self) -> Task:
        return Task(config=self.tasks_config["build_query_task"])

    @task
    def ranking_task(self) -> Task:
        return Task(config=self.tasks_config["ranking_task"])
